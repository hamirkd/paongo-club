...
export class PasswordResetComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.check_token = this.route.snapshot.paramMap.get('token');
    console.log(this.check_token)
    if (this.check_token)
      this.userService.verifyValidToken(this.check_token).subscribe(user => {
  ...
  sendPasswordLinkReset() {
    this.on_register = true;
    this.userService.sendPasswordLinkReset(this.user.email).subscribe(data => {
      this.on_register = true;
      this.toasterService.success("Veuillez consulter vos mails pour réinitialiser votre mot de passe");
      setTimeout(() => {
        this.router.navigate(['/login']);
      }, 5000);
    }, err => {
      this.on_register = false;
      this.toasterService.error("Veuillez vérifier votre adresse");
      console.log(err)
    })
  ...
  resetPassword() {
    ...
    this.userService.resetPassword(this.user).subscribe(data => {
      this.on_register = true;
      this.toasterService.success("Votre mot de passe a été réinitialisé, veuillez vous connecter maintenant");
      setTimeout(() => {
        this.router.navigate(['/login']);
      }, 5000);
    }, err => {
      this.on_register = false;
      this.toasterService.error("Impossible de réinitialiser votre mot de passe");
      console.log(err)
    })
